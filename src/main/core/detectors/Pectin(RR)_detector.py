...
"""Pectin (RR-stained region) detector for JPG images.

This script detects pectin regions in images and calculates their ratio.
...
OUTPUT_FOLDER = "src/data/detector_results/pectin_detector_results"
...
def get_pixel_microns(filename: str, nd2_lookup: dict, repo_root: Path) -> float:
    ...
    Args:
        filename: Image filename (e.g., '20240780a_T0_10xstitch_RR_nobg.jpg')
...
def detect_pectin_ratio(image: np.ndarray, cell_mask: np.ndarray = None, pixel_microns: float = None, debug: bool = False) -> Tuple[int, int, int, np.ndarray, np.ndarray]:
    ...
    Returns:
        Tuple containing:
        - pectin_count: Number of pectin pixels detected in cell region
        - cell_pixels: Total number of pixels in cell region
        - total_pixels: Total number of pixels in entire image
        - pectin_mask: Binary mask of detected pectin regions
        - visualization: Image with cell and pectin regions highlighted
    """
    if image is None:
    ...
    pectin_mask = cv2.morphologyEx(pectin_mask, cv2.MORPH_OPEN, kernel)
    pectin_mask = cv2.morphologyEx(pectin_mask, cv2.MORPH_CLOSE, kernel)

    # If cell mask is provided, only count pectin within cell region
    if cell_mask is not None:
        # Apply cell mask to pectin mask
        pectin_in_cell = cv2.bitwise_and(pectin_mask, cell_mask)
        pectin_count = int(np.count_nonzero(pectin_in_cell))
        cell_pixels = int(np.count_nonzero(cell_mask))
    else:
        # No cell mask - use entire image
        pectin_in_cell = pectin_mask
        pectin_count = int(np.count_nonzero(pectin_mask))
        cell_pixels = int(pectin_mask.shape[0] * pectin_mask.shape[1])
    
    total_pixels = int(pectin_mask.shape[0] * pectin_mask.shape[1])
    ...
    # Create blue overlay for detected pectin regions
    overlay = np.zeros_like(image)
    overlay[pectin_in_cell > 0] = [255, 0, 0]  # BGR format - pure blue
    ...
    if debug:
        print(f"pectin_count={pectin_count}, cell_pixels={cell_pixels}, total_pixels={total_pixels}, ratio={ratio:.6f}")
        print(f"pectin_square_microns={pectin_square_microns:.2f}, cell_square_microns={cell_square_microns:.2f}, pixel_microns={pixel_microns}")

    return pectin_count, cell_pixels, total_pixels, pectin_in_cell, vis_img
...
def extract_metadata_from_filename(filename: str, imageset: str) -> dict:
    ...
    Args:
        filename: Image filename (e.g., '20240780a_T0_10xstitch_RR.jpg')
        imageset: ImageSet name (e.g., '20240630-20240644_10xstitch_RR')
    ...
    if name.startswith('STANDARD'):
        ...
        # Extract from STANDARD pattern: STANDARD_20240780-20240794d_T0_10xstitch_RR
        # or: STANDARD20240780a_T0_10xstitch_RR
        ...
        # Numeric pattern: 20240780a_T0_10xstitch_RR
...
def process_folder_with_vis(input_folder: Path, yolo_model: YOLO, vis_dir: Path, conf: float = 0.25, mask_mode: str = 'auto', debug: bool = False, repo_root: Path = None) -> list:
    ...
    for idx, p in enumerate(files, 1):
        ...
        try:
            ...
            ratio = pectin_count / cell_pixels if cell_pixels > 0 else 0.0
    ...
    if results:
        ...
        if ratios:
            ...
            print(f"Average Pectin ratio: {avg_ratio:.2%}")
            print(f"Min Pectin ratio: {min_ratio:.2%}")
            print(f"Max Pectin ratio: {max_ratio:.2%}")
            print()
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description='Detect pectin (RR-stained) area in cell regions using YOLO + color detection')
    p.add_argument('--input', '-i', help='Input folder with JPG files (only for single folder mode)')
    p.add_argument('--output', '-o', help='Output CSV file path (optional, default: OUTPUT_FOLDER/combined_pectin_results.csv)')
...
def main():
    """Main function to run pectin detection"""
    try:
        ...
        if mode == 'batch':
            ...
            output_csv = Path(args.output) if args.output else (output_folder / "combined_pectin_results.csv")
        ...
        # Single folder mode
            ...
            output_csv = Path(args.output) if args.output else (Path(OUTPUT_FOLDER) / input_folder.name / "pectin_results.csv")
